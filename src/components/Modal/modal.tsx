...
const ModalComponent: React.FC<ModalComponentProps> = ({ open, onClose }) => {
    ...
    const [newTask, setNewTask] = useState<Task[]>([
        { task_type: '', content: '', date: selectedDate },
        { task_type: '', content: '', date: selectedDate },
        { task_type: '', content: '', date: selectedDate },
    ]);
        ...
        if (existingTaskIndex !== -1) {
          setNewTask((prevTasks) => {
            ...
            updatedTasks[existingTaskIndex] = {
              ...updatedTasks[existingTaskIndex],
              task_type: tasks[existingTaskIndex]?.task_type,
              content: tasks[existingTaskIndex]?.content,
              date: selectedDate,
            };
            return updatedTasks;
          });
          ...
          setTasks((prevTasks) => {
            const updatedTasks = prevTasks.map((task) => {
              if (
                ...
                return {
                  ...task,
                  task_type: validTasks[0]?.task_type || task.task_type,
                  content: validTasks[0]?.content || task.content,
                };
              }
              return task;
            });
